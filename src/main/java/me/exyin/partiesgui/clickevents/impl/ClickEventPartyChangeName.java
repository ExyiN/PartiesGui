...
public class ClickEventPartyChangeName implements ClickEvent {
  private final PartiesGui plugin;

  public ClickEventPartyChangeName(final PartiesGui plugin) {
    this.plugin = plugin;
  }
  ...
  public void execute(final PGGui gui, final Player whoClicked, final PartyPlayer partyPlayer, final int slot) {
    ...
    plugin.getMessageUtil().sendMessage(whoClicked, "settings.rename");
  }
